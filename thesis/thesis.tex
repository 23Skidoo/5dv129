...
examples, see~\cite{bib:fun}. Type-correct Haskell programs are also widely
perceived to not be wrong to begin with: when the code compiles, it often just
works~\cite{bib:wiki-works}. Type inference removes the syntactic burden of
...
Great support for concurrent and parallel programming.
...
While GHC Haskell supports a number of different parallel and concurrent
programming models, the Concurrent Haskell extension~\cite{marlow2013} is the
most relevant one for the purposes of this thesis.
